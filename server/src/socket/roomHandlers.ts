...
const joinPayload = z.object({
  roomCode: z.string().trim().length(8).transform((value) => value.toUpperCase()),
  username: z.string().trim().min(1).max(32),
  userId: z.string().uuid(),
});
const timePayload = z.object({ currentTime: z.number().finite().min(0).optional(), time: z.number().finite().min(0).optional() })
  .refine((value) => value.currentTime !== undefined || value.time !== undefined, { message: 'A valid playback time is required.' })
...
export function registerRoomSocketHandlers(io: Server) {
  io.on('connection', (socket) => {
    socket.on('join_room', async (payload: unknown) => {
      ...
      try {
        ...
        await prisma.participant.upsert({
          ...
          create: { userId: input.data.userId, roomId: room.id, username: input.data.username, socketId: socket.id },
        });
    ...
    socket.on('remove_participant', async (payload: unknown) => {
      ...
      targetSocket?.emit('participant_removed', { participantId: target.id, userId: target.userId });
      const room = await getRoom(actor.room.code);
      if (room) io.to(actor.room.code).emit('sync_state', toRoomDto(room));
    });
...
async function handleLeave(io: Server, socket: Socket, options: { deleteParticipant: boolean }) {
  ...
  // A room belongs to the room, not to the host's browser tab. When the host
  // leaves, promote the earliest connected participant so the party continues.
  if (participant.role === 'HOST') {
    const nextHost = await prisma.participant.findFirst({
      where: { roomId: participant.roomId, id: { not: participant.id }, socketId: { not: null } },
    ...
    if (nextHost) {
      await prisma.$transaction([
        ...
        ...(options.deleteParticipant ? [] : [prisma.participant.update({ where: { id: participant.id }, data: { role: 'MODERATOR', socketId: null } })]),
      ]);
    }
  }
  ...
  if (options.deleteParticipant) {
    ...
    await prisma.participant.updateMany({ where: { socketId: socket.id }, data: { socketId: null } });
  }
...
async function handlePlayback(io: Server, socket: Socket, payload: unknown, playState?: 'playing' | 'paused') {
  ...
  if (!input.success) return sendError(socket, errorCodes.INVALID_INPUT, 'currentTime must be a non-negative number.');
  const participant = await authorized(socket, ['HOST', 'MODERATOR'], 'You are not authorized to control playback.');
  if (!participant) return;
  const room = await updatePlayback(participant.roomId, { playState, currentTime: input.data.currentTime });
  io.to(participant.room.code).emit('sync_state', toRoomDto(room));
}
