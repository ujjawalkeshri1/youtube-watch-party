...
export function JoinRoomPage() {
  ...
  useEffect(() => {
    if (codeParam) setCode(normalizeRoomCode(codeParam));
  }, [codeParam]);
  ...
  const pasteInvite = async () => {
    try {
      ...
      setError('Clipboard access was blocked. Paste the invite into the room code field.');
    }
  };
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (cleanCode.length !== 8) {
      setError('Enter an 8-character room code or paste the full invite link.');
      return;
    }
    if (!username.trim()) {
      setError('Add a display name so your friends know it is you.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      setError(caught instanceof Error ? caught.message : 'Could not join that room. Check the invite and try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="auth-page">
      <div className="auth-shell">
        ...
        <section className="auth-card" style={{ boxShadow: '0 30px 100px rgba(255,51,71,.12)' }}>
          ...
          <form onSubmit={submit}>
            <div className="field-group">
              ...
              <div className="field-with-icon"><UserRound size={18} /><input id="join-username" value={username} maxLength={32} onChange={(event) => { setUsername(event.target.value); setError(''); }} placeholder="e.g. Ujjawal 😎" required /></div>
            </div>
            <div className="field-group">
            ...
            <button className="primary-action" type="submit" disabled={loading || !username.trim() || code.length !== 8}>
              {loading ? 'Joining…' : 'Join the party'}<ArrowRight size={19} />
            </button>
          </form>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 18, color: '#696972', fontSize: 11 }}>
            <span>🎬</span><span>sync'd playback</span><span>·</span><span>💬 live chat</span><span>·</span><span>🍿 good vibes</span>
          </div>
        </section>
